...
const seedDatabase = async () => {
  try {
    ...
    const phylum = await models.Phylum.create({
      name: "Động vật có dây sống",
      kingdom_id: kingdom.id,
    });
    const classMammalia = await models.Class.create({
      name: "Lớp Thú",
      phylum_id: phylum.id,
    });
    const orderCarnivora = await models.Order.create({
      name: "Bộ Ăn Thịt",
      class_id: classMammalia.id,
    });
    const familyFelidae = await models.Family.create({
      name: "Họ Mèo",
      order_id: orderCarnivora.id,
    });
    const genusPanthera = await models.Genus.create({
      name: "Chi Báo",
      family_id: familyFelidae.id,
    });
    ...
    const genusFelis = await models.Genus.create({
      name: "Chi Mèo",
      family_id: familyFelidae.id,
    });
    ...
    const phylumMagnoliophyta = await models.Phylum.create({
      name: "Ngành Ngọc Lan",
      kingdom_id: kingdomPlantae.id,
    });
    const classMagnoliopsida = await models.Class.create({
      name: "Lớp Ngọc Lan",
      phylum_id: phylumMagnoliophyta.id,
    });
    const orderRosales = await models.Order.create({
      name: "Bộ Hoa Hồng",
      class_id: classMagnoliopsida.id,
    });
    const familyRosaceae = await models.Family.create({
      name: "Họ Hoa Hồng",
      order_id: orderRosales.id,
    });
    const genusRosa = await models.Genus.create({
      name: "Chi Hoa Hồng",
      family_id: familyRosaceae.id,
    });
    ...
    const genusQuercus = await models.Genus.create({
      name: "Chi Sồi",
      family_id: familyRosaceae.id,
    });
    ...
    // --- Seed Nhóm Sinh Vật ---
    const groupMammals = await models.OrganismGroup.create({
      name: "Động vật có vú",
      symbol: "MAM",
    });
    const groupCarnivores = await models.OrganismGroup.create({
      name: "Động vật ăn thịt",
      symbol: "CAR",
    });

    // Gắn Species với Nhóm Sinh Vật
    await models.OrganismGroupSpecies.create({
      organism_group_id: groupMammals.id,
      species_id: speciesLion.id,
    });
    await models.OrganismGroupSpecies.create({
      organism_group_id: groupCarnivores.id,
      species_id: speciesLion.id,
    });
    ...
    await models.SpeciesHabitat.create({
      species_id: speciesLion.id,
      habitat_id: habitatSavanna.id,
    });
    await models.SpeciesHabitat.create({
      species_id: speciesLion.id,
      habitat_id: habitatForest.id,
    });
    await models.SpeciesHabitat.create({
      species_id: speciesRose.id,
      habitat_id: habitatDesert.id,
    });
    await models.SpeciesHabitat.create({
    ...
    // --- Seed Tác Giả ---
    const authorJane = await models.Author.create({
      name: "Jane Doe",
      affiliation: "Đại học Động vật Hoang dã",
    });
    const authorJohn = await models.Author.create({
      name: "John Smith",
      affiliation: "Viện Nghiên cứu Châu Phi",
    });

    // Gắn Tác Giả với Loài
    await models.SpeciesAuthor.create({
      species_id: speciesLion.id,
      author_id: authorJane.id,
    });
    await models.SpeciesAuthor.create({
      species_id: speciesLion.id,
      author_id: authorJohn.id,
    });
